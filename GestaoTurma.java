...
void main(String[] args) {
    ...
    int cadastrarAlunos(String[] nomes, double[] medias) {
        int quantidade;
        do {
            String entradaQtd = IO.readln("Quantos alunos deseja cadastrar? (máximo 20) ");
            if (entradaQtd.matches("\\d+")) {
                quantidade = Integer.parseInt(entradaQtd);
        ...
        for (int i = 0; i < quantidade; i++) {
            ...
            double media;
            do {
                String entradaMedia = IO.readln("Média Final: ");
                if (entradaMedia.matches("\\d+(\\.\\d+)?")) {
                    media = Double.parseDouble(entradaMedia);
